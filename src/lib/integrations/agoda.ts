/**
 * Agoda OTA Integration
 * 
 * Handles rate and availability synchronization with Agoda
...
export class AgodaIntegration {
  ...
  private getBaseUrl(): string {
    return this.sandboxMode
      ? 'https://sandbox-distribution-xml.agoda.com/api'
      : 'https://distribution-xml.agoda.com/api';
  }
  ...
  async testConnection(): Promise<OtaConnectionResult> {
    ...
    try {
      const response = await fetch(`${this.getBaseUrl()}/hotel/status`, {
        method: 'GET',
        headers: {
      ...
      if (response.ok) {
        ...
        return {
          success: true,
          message: 'Successfully connected to Agoda API',
        };
      } else {
  ...
  async pushRates(hotelId: string, roomTypeId: string, ratePlanId: string, dates: string[], rates: number[]): Promise<OtaPushResult> {
    ...
    try {
      ...
      const response = await fetch(`${this.getBaseUrl()}/rates/update`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${this.apiKey}`,
          'Content-Type': 'application/json',
  ...
  async pushAvailability(hotelId: string, roomTypeId: string, date: string, available: boolean, inventory: number = 1): Promise<OtaPushResult> {
    ...
    try {
      const payload = {
        ...
        availability_updates: [
          {
            date,
            available_rooms: available ? inventory : 0,
          },
        ],
      };

      const response = await fetch(`${this.getBaseUrl()}/inventory/update`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${this.apiKey}`,
          'Content-Type': 'application/json',
      ...
      const response = await fetch(
        `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/create-notification`,
        {
          ...
          body: JSON.stringify({
            ...
            message: `Agoda sync failed after multiple retries: ${errorMessage}`,
